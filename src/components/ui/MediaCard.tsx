...
function RatingStars({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5 mt-1" title={`${rating.toFixed(1)} / 5.0`}>
      {[0, 1, 2, 3, 4].map((index) => {
        const isFull = rating >= index + 1;
        const isHalf = rating > index && rating < index + 1;
        return (
          <div key={index} className="relative w-3.5 h-3.5">
            <Star className="absolute inset-0 w-3.5 h-3.5 text-secondary/50" strokeWidth={1.5} />
            {isFull && <Star className="absolute inset-0 w-3.5 h-3.5 text-primary fill-primary" strokeWidth={1} />}
            {isHalf && (
              <div className="absolute inset-0 overflow-hidden w-[50%] h-3.5">
                <Star className="w-3.5 h-3.5 text-primary fill-primary" strokeWidth={1} />
              </div>
            )}
          </div>
        );
    ...
    >
      ...
      <div className="flex flex-col flex-1 transform transition-transform group-hover:translate-x-1 pt-2">
        ...
        <div className="flex flex-col mt-auto pt-1">
          {cardStyle === 'vault' ? (
            rating !== undefined ? <RatingStars rating={rating} /> : null
          ) : (
            <>
