...
export function GameOfDay({ game }: GameOfDayProps) {
  ...
  const posterPath = game.coverUrl
  ...
  const handleVaultAction = async () => {
    ...
    if (isInVault) {
      ...
      try {
        await removeFromVault(game.id.toString(), "game");
        setIsInVault(false);
      } catch (err) {
  ...
  // Re-check vault status when modal closes
    ...
    <>
      <div className="md:col-span-3 group relative overflow-hidden bg-surface border border-secondary rounded-2xl flex flex-col md:flex-row items-stretch shadow-[0_10px_60px_rgba(0,0,0,0.6)] min-h-[480px] transition-all hover:border-primary/40">
        ...
        <div className="relative z-20 flex-1 p-8 md:p-12 flex flex-col justify-center bg-surface/40 backdrop-blur-sm">
          ...
          {/* Actions */}
          <div className="flex gap-4 max-w-m">
            <a
              href={`/game/${game.id}`}
              className="flex-1 bg-primary text-primary-foreground py-4 rounded-xl text-[0.7rem] font-black uppercase tracking-[0.2em] hover:bg-primary-hover active:scale-95 transition-all shadow-[0_10px_30px_rgba(208,0,0,0.2)] flex items-center justify-center text-center"
            >
              LEARN MORE
            </a>
            <button
              ...
              className="flex-1 border border-secondary text-foreground py-4 rounded-xl text-[0.7rem] font-black uppercase tracking-[0.2em] hover:bg-surface-hover hover:border-primary/50 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer"
            >
              {isActionLoading ? (
